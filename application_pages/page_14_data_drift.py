...
def main():
    ...
    if credit_score_predictor_model_id:
        st.info(f"Simulating data drift alert for model: **Credit Score Predictor** (ID: {int(credit_score_predictor_model_id)})")
        ...
        if not data_drift_risk_exists:
            ...
            st.info("The 'Data Drift' risk for Credit Score Predictor has already been simulated and updated.")
            if st.button("Proceed to Update Risk Assessment", key="next_step14_btn"):
                st.session_state.current_step = 15
                st.rerun()

    else:
        st.error("Credit Score Predictor model not found. Please ensure it is registered in Step 3.")
