...
def main():
    ...
    with st.form("add_model_form"):
        ...
        status = st.selectbox("Status", status_options, index=status_options.index("In Development"), key="status_3")
        ...
        if submitted:
            ...
            if existing_model.empty:
                ...
                st.warning(f"Model '{model_name}' is already registered (ID: {int(existing_model['model_id'].iloc[0])}). Skipping addition.")
                st.session_state.credit_score_predictor_model_id = existing_model['model_id'].iloc[0]
                st.session_state.current_step = 4
    ...
    if not st.session_state.ai_models_df.empty:
        if st.button("Proceed to Initial Risk Identification", key="next_step3_btn"):
            st.session_state.current_step = 4
            st.rerun()
