...
def main():
    ...
    if 'full_risk_register_df' not in st.session_state or st.session_state.full_risk_register_df.empty:
        st.warning("Please generate the comprehensive risk register in Step 11 first to identify top risks.")
    ...
    if 'top_risks_df' in st.session_state and not st.session_state.top_risks_df.empty:
        ...
        st.caption("These are the most critical risks identified, requiring immediate attention.")
    else:
        st.info("No top risks identified yet. Click 'Identify Top Risks' above.")
    
    if st.session_state.current_step == 12 and st.button("Proceed to Visualize Risk Distribution", key="next_step12_btn"):
        st.session_state.current_step = 13
        st.rerun()
