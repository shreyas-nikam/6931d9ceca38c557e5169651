...
def main():
    ...
    if credit_score_predictor_model_id:
        st.write(f"Adding adversarial risk for model: **Credit Score Predictor** (ID: {int(credit_score_predictor_model_id)})")
        ...
        adversarial_risk_exists = st.session_state.ai_risks_df[
            ...
            (st.session_state.ai_risks_df['hazard_description'].str.contains("Adversarial Attack", na=False))
        ].empty == False
        ...
        if not adversarial_risk_exists:
            ...
            st.info("The 'Adversarial Attack' risk for the Credit Score Predictor has already been added.")
            if st.button("Proceed to Supply Chain & Data Provenance Risks", key="next_step7_btn"):
                st.session_state.current_step = 8
                st.rerun()
