...
def main():
    ...
    if credit_score_predictor_model_id:
        ...
        if not perf_degrad_risk.empty:
            ...
            st.warning(
                f"Risk type '{target_risk_type}' not found for Credit Score Predictor. Ensure it was added in Step 4.")
            needs_update = False  # Cannot update if not found

        if needs_update:
            st.markdown(
                "Updating 'Performance Degradation' risk based on the data drift event:")
        ...
        # Show success message after update or if already updated
        if not needs_update or perf_degrad_risk.empty == False:
            st.success(
                "✅ Risk assessment updated! Proceed to Final Summary.")
