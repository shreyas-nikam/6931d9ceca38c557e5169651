    ...
    if not controls_to_respond.empty:
        ...
        with st.form("assign_responses_form"):
            for index, control in controls_to_respond.iterrows():
                ...
                effectiveness = st.slider(f"Effectiveness Score (1-5) for Control ID {control_id}", 1, 5, 4, key=f"effectiveness_{control_id}")
                risk_response_index = 0 # Default to Mitigate
                if control['risk_response'] in risk_response_options:
                    risk_response_index = risk_response_options.index(control['risk_response'])
                risk_response = st.selectbox(f"Risk Response for Control ID {control_id}", risk_response_options, index=risk_response_index, key=f"response_{control_id}")
                
                responded_controls_data.append({'control_id': control_id, 'effectiveness': effectiveness, 'risk_response': risk_response})
                st.markdown("---")
